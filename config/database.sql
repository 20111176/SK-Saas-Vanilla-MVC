-- For access Docker container need to create localhost and %  
DROP USER IF EXISTS 'sk_saas_vanilla_mvc'@'127.0.0.1';
DROP USER IF EXISTS 'sk_saas_vanilla_mvc'@'%';

CREATE USER 'sk_saas_vanilla_mvc'@'127.0.0.1'; 
CREATE USER 'sk_saas_vanilla_mvc'@'%'; 

ALTER USER 'sk_saas_vanilla_mvc'@'127.0.0.1' IDENTIFIED by 'PasswordSecret';
ALTER USER 'sk_saas_vanilla_mvc'@'%' IDENTIFIED by 'PasswordSecret';

GRANT USAGE ON *.* TO 'sk_saas_vanilla_mvc'@'127.0.0.1';
GRANT USAGE ON *.* TO 'sk_saas_vanilla_mvc'@'%';

GRANT ALL PRIVILEGES ON `sk_saas_vanilla_mvc`.* to 'sk_saas_vanilla_mvc'@'127.0.0.1';
GRANT ALL PRIVILEGES ON `sk_saas_vanilla_mvc`.* to 'sk_saas_vanilla_mvc'@'%';


# DATA base setup
DROP DATABASE if EXISTS `sk_saas_vanilla_mvc`;

CREATE DATABASE IF NOT EXISTS `sk_saas_vanilla_mvc`;


USE sk_saas_vanilla_mvc;
...
CREATE TABLE IF NOT EXISTS `products` (
	...
	`created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
	PRIMARY KEY (`id`)) engine = InnoDB AUTO_INCREMENT = 1000 DEFAULT charset = utf8mb4 COLLATE = utf8mb4_general_ci;
 

-- seeding
-- insert data into 'users' TABLE

-- the password is password1 hashed using the php password_hash()
...
VALUES (20, 'Adrian Gould', 'adrian@example.com',
       ...
       (30, 'Cedric Ko', 'cedric@example.com',
        '$2y$10$4Ae3n2iQ0MwXMNz0UEmNne2PaNyfYsBFYb97nayHWTDCwpnuPi6f.',
